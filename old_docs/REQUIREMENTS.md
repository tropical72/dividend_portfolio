...
### 3.2. 포트폴리오 관리 (Portfolio Tab)
- **카테고리 시스템**:
    - **고정인컴(Fixed Income):** 리츠, 채권 ETF 등 매월/매분기 안정적 배당 종목.
    - **배당성장(Dividend Growth):** 배당금이 매년 인상되는 우량주.
    - **기타/버퍼(Others/Cash):** 현금, 단기 채권 등 변동성 방어용 자산.
- **종목 이관 UX**:
    - Watchlist 테이블에서 종목을 선택하여 특정 카테고리로 추가할 수 있는 버튼 또는 메뉴 제공.
- **입력 및 계산 필드**:
    - **수량(Quantity):** 소수점 지원 (미국주식 소수점 매매 대응).
    - **평균 단가(Avg Price):** 해당 종목의 매입 평균가.
    - **비중(%):** 전체 포트폴리오 자산 대비 해당 종목의 평가 금액 비율 (자동 계산).
- **데이터 업데이트**:
    - 포트폴리오 내 종목의 현재가는 Watchlist의 실시간 데이터를 공유하여 갱신.
- **검증 로직**:
    - 각 카테고리별 비중 합계 및 전체 비중 합계(100%) 표시.
    - 100% 초과 시 사용자에게 시각적 경고(텍스트 색상 변경 등) 제공.
