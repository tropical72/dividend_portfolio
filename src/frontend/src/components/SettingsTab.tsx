        ...
        <div className="lg:col-span-8 space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <section className="bg-slate-900/40 p-8 rounded-[2.5rem] border border-slate-800 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4 border-t border-slate-800 pt-6">
                <InputGroup label="Private Pension" value={retireConfig.user_profile.private_pension_start_age} onChange={(v) => setRetireConfig({...retireConfig, user_profile: {...retireConfig.user_profile, private_pension_start_age: parseInt(v)}})} />
                <InputGroup label="National Pension" value={retireConfig.user_profile.national_pension_start_age} onChange={(v) => setRetireConfig({...retireConfig, user_profile: {...retireConfig.user_profile, national_pension_start_age: parseInt(v)}})} />
              </div>
            </section>
            ...
            <section className="bg-slate-900/40 p-8 rounded-[2.5rem] border border-slate-800 space-y-6">
              ...
              <InputGroup label="Initial Capital" value={retireConfig.pension_params.initial_investment} isCurrency tooltip="현재 연금 계좌 기초 자산 총액" onChange={(v) => setRetireConfig({...retireConfig, pension_params: {...retireConfig.pension_params, initial_investment: parseInt(v)}})} />
              <div className="grid grid-cols-2 gap-4">
                <InputGroup label="Severance" value={retireConfig.pension_params.severance_reserve} isCurrency tooltip="예상 퇴직금 총액" onChange={(v) => setRetireConfig({...retireConfig, pension_params: {...retireConfig.pension_params, severance_reserve: parseInt(v)}})} />
                <InputGroup label="Other" value={retireConfig.pension_params.other_reserve} isCurrency tooltip="기타 여유 자산" onChange={(v) => setRetireConfig({...retireConfig, pension_params: {...retireConfig.pension_params, other_reserve: parseInt(v)}})} />
              </div>
            </section>
          </div>
          ...
          <section className="bg-slate-900/40 p-8 rounded-[2.5rem] border border-slate-800 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <InputGroup label="Total Inv." value={retireConfig.corp_params.initial_investment} isCurrency onChange={(v) => setRetireConfig({...retireConfig, corp_params: {...retireConfig.corp_params, initial_investment: parseInt(v)}})} />
              <InputGroup label="Capital" value={retireConfig.corp_params.capital_stock} isCurrency onChange={(v) => setRetireConfig({...retireConfig, corp_params: {...retireConfig.corp_params, capital_stock: parseInt(v)}})} />
              <InputGroup label="Loan" value={retireConfig.corp_params.initial_shareholder_loan} isCurrency onChange={(v) => setRetireConfig({...retireConfig, corp_params: {...retireConfig.corp_params, initial_shareholder_loan: parseInt(v)}})} />
            </div>
          </section>
              ...
              {(!retireConfig.planned_cashflows || retireConfig.planned_cashflows.length === 0) ? 
                ...
                retireConfig.planned_cashflows.map((ev) => (
                  <div key={ev.id} className="bg-slate-950/50 p-6 rounded-3xl border border-slate-800 space-y-6 group relative">
                    {/* 1행: 모든 요소의 높이를 h-11로 통일하여 수평 정렬 완성 */}
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                      <div className="md:col-span-3 flex flex-col gap-2">
                        <label className="text-[10px] font-black text-slate-500 uppercase ml-1">Type & Target</label>
                        <div className="flex gap-2">
                          <select value={ev.type} onChange={(e) => updateCashflow(ev.id, {type: e.target.value as any})} className="h-11 flex-1 bg-slate-900 border border-slate-800 rounded-lg px-2 text-[10px] font-black uppercase text-slate-300 outline-none">
                            <option value="INFLOW">In (+)</option>
                            <option value="OUTFLOW">Out (-)</option>
                          </select>
                          <select value={ev.entity} onChange={(e) => updateCashflow(ev.id, {entity: e.target.value as any})} className="h-11 flex-1 bg-slate-900 border border-slate-800 rounded-lg px-2 text-[10px] font-black uppercase text-slate-300 outline-none">
                      ...
                      <div className="md:col-span-5 flex flex-col gap-2">
                        ...
                        <div className="flex items-center gap-2">
                          <select 
                            ...
                            onChange={(e) => updateCashflow(ev.id, {currency: e.target.value as any})} 
                            className="h-11 w-28 bg-slate-800 border border-slate-700 rounded-xl px-2 text-[10px] font-black text-slate-400 outline-none flex-shrink-0"
                          >
                          ...
                          <input 
                            ...
                            className="h-11 w-full bg-slate-900 border border-slate-800 rounded-xl px-4 text-base font-black text-emerald-400 outline-none focus:border-emerald-500/50" 
                          />
                        </div>
                      </div>
                      ...
                      <div className="md:col-span-2 flex flex-col gap-2">
                        ...
                        <input type="number" value={ev.year} onChange={(e) => updateCashflow(ev.id, {year: parseInt(e.target.value)})} className="h-11 w-full bg-slate-900 border border-slate-800 rounded-xl px-3 text-sm font-black text-slate-200 text-center outline-none focus:border-blue-500/50" />
                      </div>

                      <div className="md:col-span-2 flex flex-col gap-2">
                        <label className="text-[10px] font-black text-slate-500 uppercase ml-1">Month</label>
                        <input type="number" value={ev.month} onChange={(e) => updateCashflow(ev.id, {month: parseInt(e.target.value)})} className="h-11 w-full bg-slate-900 border border-slate-800 rounded-xl px-3 text-sm font-black text-slate-200 text-center outline-none focus:border-blue-500/50" />
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-2 pt-4 border-t border-slate-900">
                      ...
                      <div className="flex items-center gap-4">
                        <input 
                          ...
                          className="flex-1 bg-slate-900/50 border border-slate-800/50 rounded-xl px-4 py-3 text-sm font-medium text-slate-400 placeholder:text-slate-700 outline-none focus:border-slate-600 focus:bg-slate-900 transition-all" 
                          placeholder="상세 내용을 입력하세요" 
                        />
                        <button onClick={() => removeCashflow(ev.id)} className="p-3 text-slate-600 hover:text-rose-500 bg-slate-900 hover:bg-rose-500/10 rounded-xl transition-all border border-slate-800 hover:border-rose-500/30 flex-shrink-0">
                          <Trash2 size={20} />
                        </button>
                      </div>
                    </div>
          ...
          <section className="bg-slate-900/40 p-8 rounded-[2.5rem] border border-slate-800 space-y-6">
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                <input type="number" step="0.1" value={retireConfig.tax_and_insurance?.point_unit_price || 208.4} onChange={(e) => setRetireConfig({...retireConfig, tax_and_insurance: {...retireConfig.tax_and_insurance, point_unit_price: parseFloat(e.target.value)}})} className="w-full bg-slate-950/50 border border-slate-800 rounded-xl px-4 py-3 text-sm font-black text-slate-200 outline-none focus:border-emerald-500" />
              </div>
              <InputGroup label="SGOV Buffer" value={retireConfig.trigger_thresholds?.target_buffer_months || 24} onChange={(v) => setRetireConfig({...retireConfig, trigger_thresholds: {...retireConfig.trigger_thresholds, target_buffer_months: parseInt(v)}})} />
            </div>
          </section>
...
function InputGroup({ label, value, onChange, isCurrency = false, tooltip }: { label: string, value: number, onChange: (v: string) => void, isCurrency?: boolean, tooltip?: string }) {
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="relative">
        <input type="text" value={displayValue} onChange={(e) => { const rawValue = e.target.value.replace(/,/g, ""); onChange(rawValue === "" ? "0" : rawValue); }} className="w-full bg-slate-950/50 border border-slate-800 rounded-xl px-4 py-2.5 text-sm font-black text-slate-200 outline-none focus:border-emerald-500 transition-all" />
        {isCurrency && <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-black text-slate-600">KRW</span>}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="flex items-center gap-3">
        <input type="text" className="flex-1 bg-slate-900 border border-slate-800 rounded-xl px-3 py-2 text-sm font-black text-slate-200 outline-none focus:border-blue-500/50" value={value} onChange={(e) => setValue(e.target.value)} onBlur={() => !isNaN(parseFloat(value)) && onCommit(parseFloat(value))} onKeyDown={(e) => e.key === "Enter" && (e.target as any).blur()} />
