...
*   **Quality Gate (MUST):** 커밋 전에는 변경 범위 기준으로 아래 검증을 모두 통과해야 한다.
    *   Python 변경 포함 시: `PYTHONPATH=. .venv/bin/ruff check <changed_python_files>`, `PYTHONPATH=. .venv/bin/black --check <changed_python_files>`, 관련 `pytest`.
    *   Frontend 변경 포함 시: `npm run lint`, `npm run build`, `npx prettier --check <changed_frontend_files>`, 관련 `Playwright`.
    *   **레거시 예외 관리:** 저장소 전체에 남아 있는 기존 `black`/`mypy` 오류는 별도 전역 정비 태스크로 추적하되, 신규 변경분이 해당 오류를 늘리거나 방치한 채 커밋해서는 안 된다.
...
## Development Context (Last Updated: 2026-04-24)

### 1. 현재 상태 Summary
*   **Cost Comparison Simulator 1차 구현 완료**: 개인운용 vs 법인운용 비교 탭, 기준 포트폴리오 연동, KPI/차트/감사(Audit) 패널, 정책 메타 표시까지 문서 기준 주요 범위가 구현됨.
*   **Asset-driven 확장 반영**: Target-driven 외에 보유 자산 기반 비교 모드와 30년 장기 프로젝션이 추적 문서에 반영되어 있으며, 관련 테스트 ID(`TEST-CCS-90`~`95`)가 정의됨.
*   **문서 구조 동기화 진행 중**: `GEMINI.md`, `trace_matrix.md`, 도메인 요구사항 문서의 경로/구조 불일치를 실제 저장소 구조 기준으로 정리 중임.

### 2. 차기 작업 우선순위 (CRITICAL NEXT)
*   **[D-CCS-9.3.3] Asset-driven 모드 wiring 및 KPI 단위 회귀 수정**:
    *   **목표**: UI 저장/실행 경로와 백엔드 `simulation_mode` 처리 경로를 일치시키고, Asset-driven KPI 카드가 월 기준 값을 정확히 표시하도록 회귀를 제거한다.
    *   **핵심**: `TEST-CCS-94`를 통과시키는 수준까지 입력 저장, 실행 응답, 화면 표기 단위를 end-to-end로 정합화한다.
*   **[D-CCS-9.3.4] 개인 Asset-driven 장기 시계열 이중 계산 회귀 수정**:
    *   **목표**: 개인 시나리오 장기 프로젝션에서 동일 세후 현금이 누적 가치에 중복 반영되지 않도록 계산 경계를 바로잡는다.
    *   **핵심**: `TEST-CCS-95` 기준으로 `total_economic_value`와 자산 증가/가계현금 누적 로직을 분리 검증한다.
