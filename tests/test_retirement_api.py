...
def test_retirement_api_dynamic_rebalance_month_applies_inflation_from_next_month(
    tmp_path, monkeypatch
):
    backend = DividendBackend(data_dir=str(tmp_path), ensure_default_master_bundle=True)
    monkeypatch.setattr(main_module, "backend", backend)
    ...
    local_client.post(
        ...
        json={
            ...
            "strategy_rules": {"rebalance_month": 6},
        },
    )
    ...
    assert june["next_target_cashflow"] == pytest.approx(1100000.0)
    assert july["target_cashflow"] == pytest.approx(1100000.0)
...
def test_retirement_api_boost_ladder_follows_shock_drawdown_ranges(
    tmp_path, monkeypatch, monthly_pa, expected_boost
):
    backend = DividendBackend(data_dir=str(tmp_path), ensure_default_master_bundle=True)
    monkeypatch.setattr(main_module, "backend", backend)
    local_client = TestClient(main_module.app)
    ...
    backend.portfolios = [
        {
            "id": "corp",
            "name": "Boost Ladder Corporate",
            "account_type": "Corporate",
            "total_capital": 100000000,
        },
        {
            "id": "pension",
            "name": "Boost Ladder Pension",
            "account_type": "Pension",
...
def test_retirement_api_stress_without_crash20_still_triggers_first_boost_ladder(
    tmp_path, monkeypatch
):
    backend = DividendBackend(data_dir=str(tmp_path), ensure_default_master_bundle=True)
    monkeypatch.setattr(main_module, "backend", backend)
    ...
    backend.portfolios = [
        {
            "id": "corp",
            "name": "Stress Boost Corporate",
            "account_type": "Corporate",
            "total_capital": 100000000,
        },
        {
            "id": "pension",
            "name": "Stress Boost Pension",
            "account_type": "Pension",
            "total_capital": 50000000,
        },
    ]
...
def test_retirement_api_stress_boost_follows_dynamic_rebalance_month(tmp_path, monkeypatch):
    backend = DividendBackend(data_dir=str(tmp_path), ensure_default_master_bundle=True)
    monkeypatch.setattr(main_module, "backend", backend)
    local_client = TestClient(main_module.app)
    ...
    backend.portfolios = [
        {
            "id": "corp",
            "name": "Dynamic Stress Boost Corporate",
            "account_type": "Corporate",
            "total_capital": 100000000,
        },
        {
            "id": "pension",
            "name": "Dynamic Stress Boost Pension",
            "account_type": "Pension",
            "total_capital": 50000000,
        },
    ]
    ...
    june = next(m for m in payload["data"]["monthly_data"] if m["year"] == 2026 and m["month"] == 6)
    july = next(m for m in payload["data"]["monthly_data"] if m["year"] == 2026 and m["month"] == 7)
...
def test_retirement_api_boost_persists_for_six_months_and_then_reverts(tmp_path, monkeypatch):
    backend = DividendBackend(data_dir=str(tmp_path), ensure_default_master_bundle=True)
    monkeypatch.setattr(main_module, "backend", backend)
    local_client = TestClient(main_module.app)
    ...
    backend.portfolios = [
        {
            "id": "corp",
            "name": "Boost Persistence Corporate",
            "account_type": "Corporate",
            "total_capital": 100000000,
        },
        {
            "id": "pension",
            "name": "Boost Persistence Pension",
            "account_type": "Pension",
            "total_capital": 50000000,
        },
    ]
    ...
    june = monthly[(2026, 6)]
    july = monthly[(2026, 7)]
    november = monthly[(2026, 11)]
    december = monthly[(2026, 12)]

    assert june["boost_amount"] == 3000000.0
    assert july["pension_draw"] == pytest.approx(5500000.0)
    assert november["pension_draw"] == pytest.approx(5500000.0)
    assert december["boost_amount"] == 0.0
...
def test_retirement_simulation_applies_non_standard_profile_return_override(tmp_path, monkeypatch):
    backend = DividendBackend(data_dir=str(tmp_path), ensure_default_master_bundle=True)
    corp_portfolio = backend.add_portfolio(
