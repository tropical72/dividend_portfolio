...
- **[REQ-RAMS-2.3] 법인 현금흐름과 주주대여금 반환 분리:**
  ...
  - **[REQ-RAMS-2.3.7] 대여금 소진 후 과세 지급:** 주주대여금 잔액이 가계 부족액보다 작으면 잔여 부족액은 과세 주주분배 추정 경로로 지급한다. 엔진은 설정된 원천징수 추정률로 순수령 부족액을 gross-up하여 법인 SGOV 현금유출과 가계 실수령을 분리 기록해야 한다. 이 값은 확정 세무자문이 아닌 장기 현금흐름 추정치임을 UI에 표시한다.
...
- **[REQ-RAMS-3.2] 동적 인출 및 리밸런싱 알고리즘:**
  ...
  - **[REQ-RAMS-3.2.2] 단일 가계 필요금액:** `household_monthly_need`를 모든 전략이 공유하는 유일한 가계 인출 목표로 사용한다. Corporate/Personal/Pension별 별도 인출 목표는 가계 목표로 사용하지 않는다. 월 기장비와 연 세무조정료는 법인 운영비로 별도 현금유출 처리한다. 레거시 `target_monthly_cashflow`는 `household_monthly_need` 호환 별칭으로만 유지한다.
  - **[REQ-RAMS-3.2.2a] 지급 우선순위:** 월 가계 필요액은 `국민연금 -> 개인연금 수령액 -> 활성 주 운용계좌` 순서로 충당한다. 개인연금 월 금액은 독립 가계 목표가 아니라 연금 개시 후 가용한 수입원이다.
  ...
  - **[REQ-RAMS-3.2.12] 법인 실현소득 과세 기준:** 법인세 과세표준은 월별 배당/이자/인컴과 실제 리밸런싱 매도의 실현차익 등 `실현소득`만 누적하여 계산해야 하며, `PA`로 반영되는 미실현 자산가격 상승분은 법인세 과세표준에 포함하면 안 된다.
  ...
  - **[REQ-RAMS-3.2.18] 개인 일반계좌 현금흐름 경계:** Personal이 주 운용계좌이면 `personal_account_params.monthly_withdrawal_target`을 계산에 사용하지 않는다. 실제 월 인출액은 연금·국민연금 차감 후 남은 가계 부족액으로만 결정한다. 레거시 저장값은 읽을 수 있으나 UI에서 활성 입력으로 노출하지 않는다.
  ...
  - **[REQ-RAMS-3.2.25] 금융소득 종합과세 비교과세 추정:** 미국 배당과 사용자 입력 외부 금융소득 합계가 설정 기준을 초과하면 소득세법 제62조 구조에 따라 `일반산출세액 = 누진세액(기타 종합소득 과세표준 + 기준 초과 금융소득) + 기준금액 × 국내 배당세율`, `비교산출세액 = 누진세액(기타 종합소득 과세표준) + 전체 금융소득 × 국내 배당세율`을 계산하고 큰 금액을 선택한다. 선택 세액에서 기타 종합소득 단독 세액을 뺀 금융소득 증분세액을 미국 배당 비중으로 배분한 뒤 외국납부세액공제를 적용한다. 일반·비교·증분 세액을 감사값으로 제공하고 결과를 추정치로 명시한다.
  ...
  - **[REQ-RAMS-3.2.28] 연도별 개인 세금 감사표:** 엔진은 세금연도별 총배당, 미국 원천세, 외국납부세액공제, 일반산출세액, 비교산출세액, 금융소득 증분세액, 국내 추가세, 매도대금, 매도원가, 실현손익, 연간 기본공제, 양도세, 지역건보료 합계, 납부연월, 연말 취득원가를 단일 감사 레코드로 제공해야 한다.
  - **[REQ-RAMS-3.2.29] 세금 재원 매도 추적:** 세금·원천세·건보료 납부를 위해 발생한 donor 매도 이벤트는 일반 리밸런싱 매도와 구분되는 `cash_obligation` 사유를 기록하고 감사표에서 납부 종류, 매도 카테고리, 매도대금, 취득원가, 실현손익을 연결해 표시해야 한다.
  - **[REQ-RAMS-3.2.30] 활성 계좌 예정 현금흐름 경계:** 예정 현금흐름은 대상 계좌가 현재 활성 마스터 전략에 포함된 경우에만 적용한다. 비활성 Corporate/Pension 계좌에는 유입·유출 이벤트를 생성하지 않는다.
  - **[REQ-RAMS-3.2.31] 개인운용 자동 가계 인출:** Personal이 활성이고 Corporate가 비활성이면 월 개인 인출 목표는 `max(0, 가계필요액 - 개인연금 실수령 - 국민연금 실수령)`으로 계산한다. 실제 인출액은 Personal SGOV 가용액으로 제한하며 가계 현금 유입과 취득원가/거래 원장에 반영한다.
  - **[REQ-RAMS-3.2.33] 주 운용계좌 공통 버퍼 규칙:** Corporate와 Personal 중 활성화된 계좌를 Operating Account로 정의한다. Operating Account는 계좌 법적 형태와 무관하게 5월 SGOV 30개월, 11월 SGOV 27개월, Bond floor/target/upper 12/18/24개월과 동일 donor 순서를 사용한다. Pension은 보조 수입계좌로서 기존 24개월 SGOV 규칙을 유지한다.
  - **[REQ-RAMS-3.2.32] 가계 현금흐름 완전성 KPI:** 엔진은 누적 가계 필요액, 누적 실지급액, 누적 미충족액, 최초 미충족 연월을 요약 응답으로 제공해야 한다. 최종자산은 이 KPI와 함께 해석되어야 한다.
...
- **[REQ-RAMS-3.4] Shock / Stress / Inflation 운영 규칙:**
  ...
  - **[REQ-RAMS-3.4.6] 개인운용 5월 판정:** Corporate가 비활성이고 Personal이 활성인 전략은 Personal SGOV 목표·Bond floor와 Personal/Pension 활성자산을 기준으로 A/B/C 테스트를 수행한다. 비활성 Corporate 자산을 이유로 Stress를 강제하거나 인플레이션을 동결해서는 안 된다.
...
- **[REQ-RAMS-8.7] 개인운용 초기자산 경고:** Personal이 포함된 활성 마스터 전략에서 `personal_account_params.initial_investment`가 0이면 Retirement 결과 화면은 해당 포트폴리오 비중이 실제 운용원금 없이 적용되고 있음을 명시적으로 경고해야 한다.
- **[REQ-RAMS-8.8] 인출 설정 단일화:** Settings는 `월 가계필요비용`만 가계 인출 목표로 노출한다. 개인 일반계좌의 별도 월 인출 입력은 제거하며, 개인연금 월 금액은 `월 개인연금 수령액`으로 명명하여 수입원임을 명확히 한다.
- **[REQ-RAMS-8.9] 개인 취득원가 설명:** `initial_cost_basis`는 현재 평가액과 별개의 세무상 원화 취득원가이며 리밸런싱/생활비/세금 재원 매도의 비례 매도원가, 실현손익, 양도세와 연말 잔여원가에만 영향을 준다. 자산 수익률이나 초기 평가액 자체를 변경하지 않는다.
