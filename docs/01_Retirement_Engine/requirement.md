...
- **[REQ-RAMS-3.2] 동적 인출 및 리밸런싱 알고리즘:**
  ...
  - **[REQ-RAMS-3.2.28] 연도별 개인 세금 감사표:** 엔진은 세금연도별 총배당, 미국 원천세, 외국납부세액공제, 국내 추가세, 매도대금, 매도원가, 실현손익, 연간 기본공제, 양도세, 지역건보료 합계, 납부연월, 연말 취득원가를 단일 감사 레코드로 제공해야 한다.
  ...
  - **[REQ-RAMS-3.2.31] 개인운용 자동 가계 인출:** Personal이 활성이고 Corporate가 비활성이면 월 개인 인출 목표는 `max(사용자 설정 최소 인출액, 가계필요액 - 개인연금 실수령 - 국민연금 실수령)`으로 계산한다. 실제 인출액은 Personal SGOV 가용액으로 제한하며 가계 현금 유입과 취득원가/거래 원장에 반영한다.
