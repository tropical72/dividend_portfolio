...
- **[REQ-WCH-04.6] 한국 종목 한글 이름 우선순위:** 
    - 한국 종목(.KS, .KQ)은 yfinance에서 제공하는 영문 이름보다 네이버 금융 또는 DART에서 수집한 한글 이름을 최우선으로 표시함.
    - 외부 API 통합 과정에서 한글 이름이 영문으로 덮어씌워지지 않도록 보호 로직을 적용함.
