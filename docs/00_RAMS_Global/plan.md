...
- **T-GLB-13.4: [Validation] 정합성 검증**
    - [x] **T-GLB-13.4.1** 자산군별 PA 차등 적용 시 시뮬레이션 결과의 합리적 차이(SGOV 정체 vs 성장주 우상향) 검증.
    - [x] **T-GLB-13.4.2** 국민연금 입력값 변경 시 65세 이후 그래프 곡선 변화 확인.
...
### [Phase 16] 자산군별 PA 시나리오 3중화 (Phase 1) [DONE]
- **T-GLB-16.1: [Backend] 설정 스키마 및 계산 경로 시나리오화**
    - [x] **T-GLB-16.1.1** `settings.json`의 `appreciation_rates`를 `conservative / base / optimistic` 중첩 구조로 확장하고 `default_pa_scenario`를 추가한다.
    - [x] **T-GLB-16.1.2** 레거시 단일 PA 구조를 `base` 시나리오로 정규화하는 호환 계층을 추가한다.
    - [x] **T-GLB-16.1.3** `get_portfolio_stats_by_id`, `calculate_master_portfolio_tr`, `retirement simulate`, `cost comparison`이 선택된 PA 시나리오를 받아 계산하도록 수정한다.
- **T-GLB-16.2: [Frontend] Settings 시나리오 편집 UI**
    - [x] **T-GLB-16.2.1** Settings에 3개 시나리오 선택/편집 UI와 기본 시나리오 선택 UI를 추가한다.
    - [x] **T-GLB-16.2.2** 각 자산군 툴팁에 기준 종목(SGOV/VGIT/JEPI-Syle/SCHD/S&P500)을 명시한다.
- **T-GLB-16.3: [Frontend] 핵심 화면 선택형 PA 시나리오**
    - [x] **T-GLB-16.3.1** Portfolio Manager(Designer + Dashboard)에 시나리오 선택기를 추가하고 TR 표시를 선택값 기준으로 갱신한다.
    - [x] **T-GLB-16.3.2** Retirement 탭에 시나리오 선택기를 추가하고 시뮬레이션/메타/TR 표시가 선택값을 따르도록 연결한다.
    - [x] **T-GLB-16.3.3** Cost Comparison 탭에 시나리오 선택기를 추가하고 결과 `DY / PA / TR` 및 비교 계산이 선택값을 따르도록 연결한다.
- **T-GLB-16.4: [Validation] Phase 1 회귀 검증**
    - [x] **T-GLB-16.4.1** 레거시 설정 로드 시 신규 3시나리오 스키마로 정규화되는지 검증한다.
...
### [Phase 18] 테스트 인프라 정상화 (T-GLB-18)

- [x] **T-GLB-18.1** 삭제된 Kivy UI 테스트를 제거하고 React/Playwright 계약을 단일 UI E2E 기준으로 확정한다.
- [x] **T-GLB-18.2** API 테스트가 기본 시드와 전역 backend 상태에 의존하지 않도록 격리한다.
- [x] **T-GLB-18.3** 기본 watchlist 개수 하드코딩을 제거하고 증분 계약으로 검증한다.
- [x] **T-GLB-18.4** legacy 월 생활비 필드가 내장 기본값보다 우선하도록 설정 병합을 정규화한다.
- [x] **T-GLB-18.5** 전체 pytest와 품질 게이트를 통과한다.
