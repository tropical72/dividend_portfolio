...
- **[REQ-SYS-02.3] 테스트 자동 실행:** 코드 변경 시 Antigravity CLI가 직접 테스트 명령어를 실행하고 결과를 분석하는 워크플로우 지원.
...
- **[REQ-SYS-05.6] 사용자 상태 메시지 키 관리:** 토스트, 에러, 경고, 확인 모달에 노출되는 문구는 화면 내부 한/영 분기 객체가 아니라 `i18n.tsx`의 도메인별 번역 키에서 가져와야 한다.
