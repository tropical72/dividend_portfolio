...
- **T-02-2.1: 설계 인터페이스 구축**
  ...
  - [x] **T-02-2.1.2** 실시간 비중 합산 및 100% 검증/저장 차단 로직. (REQ-PRT-01.3, 01.4, 01.5)
    - [x] **T-02-2.1.3** SGOV/Bond Buffer 항목에 개월 수 입력 보조필드를 제공하고 월 필요현금 기준으로 비중(%)을 환산한다. (REQ-PRT-01.9)
- **T-02-2.2: 시뮬레이션 패널**
  - [x] **T-02-2.2.1** 투자금 통화 이원화 입력 및 환율 실시간 연동. (REQ-PRT-03.1, 03.2)
  - [x] **T-02-2.2.1a** 설계 화면 시뮬레이션 투자금 기본값을 은퇴 설정의 계좌별 KRW 투자금 기준으로 전환한다. (REQ-PRT-03.1.1)
  - [x] **T-02-2.2.2** 카테고리별/전체 분석 결과 리포트 출력. (REQ-PRT-03.4)
  - [x] **T-02-2.2.3** 설계 중인 포트폴리오의 월별 배당금 지급액 막대 그래프를 추가한다. (REQ-PRT-03.6)
...
- **T-02-4.2: 월별 수입 시각화**
  ...
  - [x] **T-02-4.2.2** 마스터 전략 다중 선택을 추가하고, 법인/연금 구성 포트폴리오의 월별 배당금을 합산해 기존 비교 그래프에 함께 렌더링한다. (REQ-PRT-06.4.1)
- **T-02-4.3: 전역 시뮬레이터**
...
### [Phase 12] 법인/개인 운용 배타적 마스터 전략 (T-02-12)

- [x] **T-02-12.1** Corporate와 Personal을 같은 마스터 전략에 넣지 않는 SDD 계약을 확정한다.
- [x] **T-02-12.2** 생성·수정·활성화 API의 혼합 전략 차단 실패 테스트를 작성한다.
- [x] **T-02-12.3** 백엔드 공통 검증과 프론트엔드 상호배타 선택 UI를 구현한다.
