...
### [Phase 5] UI 가독성 개선 및 명칭 통일 (T-02-5)
- **T-02-5.1: 명칭 전환 (Personal -> Corporate)**
    - [ ] **T-02-5.1.1** `PortfolioTab.tsx` 상태 변경 및 렌더링 텍스트 업데이트.
    - [ ] **T-02-5.1.2** 백엔드 API 호환성 및 `portfolios.json` 저장 로직 검증.
- **T-02-5.2: UI 가독성 고도화**
    - [ ] **T-02-5.2.1** 서브 탭 내비게이션(Designer/Manage) 크기(`text-sm`) 및 패딩 확대.
    - [ ] **T-02-5.2.2** 계좌 타입 선택기 크기(`text-sm`) 및 선택 상태 색상 대비 강력 개선.
